...
Baseline_output_path <- "Rasters/R_Humidity/Baseline/"
Future_output_path <- "Rasters/R_Humidity/Future/"

#BASELINE
# Open the netCDF file
nc_file5 <- nc_open("Inputs/NetCDFs/1981-2000_July_RHumidity.nc")

# Get the projection coordinates
lon <- ncvar_get(nc_file5, "projection_x_coordinate")
lat <- ncvar_get(nc_file5, "projection_y_coordinate")

# Get the RHvariable data
variable_data <- ncvar_get(nc_file5, "hurs")

# Get the ensemble member names
ensemble_members <- ncvar_get(nc_file5, "ensemble_member")
...
for (i in 1:length(raster_list)) {
  ...
  filename <- paste0(Baseline_output_path, "E", ensemble_members[i], "_bRHumidity.tif")
...
#FUTURE
# Open the netCDF file
nc_file6 <- nc_open("Inputs/NetCDFs/2061-2080_July_RHumidity.nc")

# Get the projection coordinates
lon <- ncvar_get(nc_file6, "projection_x_coordinate")
lat <- ncvar_get(nc_file6, "projection_y_coordinate")

# Get the RHvariable data
variable_data <- ncvar_get(nc_file6, "hurs")

# Get the ensemble member names
ensemble_members <- ncvar_get(nc_file6, "ensemble_member")
...
for (i in 1:length(raster_list)) {
  ...
  # Define the filename using the ensemble member name
  filename <- paste0(Future_output_path, "E", ensemble_members[i], "_fRHumidity.tif")
...
###TRIAL
DIDITWORK <- raster("Rasters/R_Humidity/Future/E7_fRHumidity.tif") 
plot(DIDITWORK)
